...
from cosmpy.tx.interface import TxInterface, AsyncTxInterface
...
class AsyncTxRestClient(AsyncTxInterface):
    """Tx REST client."""

    API_URL = "/cosmos/tx/v1beta1"

    def __init__(self, rest_client: AsyncRestClient) -> None:
        """
        Create a Tx rest client.
    ...
    async def get_tx(self, request: GetTxRequest) -> GetTxResponse:
        ...
        :return: GetTxResponse
        """
        response = await self.rest_client.get(f"{self.API_URL}/txs/{request.hash}")
    ...
    async def broadcast_tx(self, request: BroadcastTxRequest) -> BroadcastTxResponse:
        """
        BroadcastTx broadcast transaction.
        ...
        :return: BroadcastTxResponse
        """
        response = await self.rest_client.post(f"{self.API_URL}/txs", request)
        return Parse(response, BroadcastTxResponse())
    ...
    async def get_txs_event(self, request: GetTxsEventRequest) -> GetTxsEventResponse:
        ...
        :return: GetTxsEventResponse
        """
        response = await self.rest_client.get(f"{self.API_URL}/txs", request)
