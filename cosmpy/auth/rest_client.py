...
from cosmpy.auth.interface import Auth, AsyncAuth
...
class AsyncAuthRestClient(AsyncAuth):
    """Auth REST client."""

    API_URL = "/cosmos/auth/v1beta1"

    def __init__(self, rest_api: AsyncRestClient):
        """
        Initialize authentication rest client.
    ...
    async def account(self, request: QueryAccountRequest) -> QueryAccountResponse:
        ...
        :return: QueryAccountResponse
        """
        json_response = await self._rest_api.get(f"{self.API_URL}/accounts/{request.address}")
        return Parse(json_response, QueryAccountResponse())
    ...
    async def params(self, request: QueryParamsRequest) -> QueryParamsResponse:
        ...
        :return: QueryParamsResponse
        """
        json_response = await self._rest_api.get(f"{self.API_URL}/params")
        return Parse(json_response, QueryParamsResponse())
